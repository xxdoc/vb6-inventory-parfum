...
Private Sub awal()
    ...
    Me.lblSubHead = "Data Supplier"     'set sub title form
...
Private Sub cmdReload_Click()
    ...
    Dim strWhere As String
        strWhere = ""
        If Me.txtCari <> "" Then
            strWhere = " WHERE nama_supplier like '%" & modulGencil.AntiSQLi(Me.txtCari) & "%'"
        End If
    'paging
    ...
    sql = "SELECT * FROM " & strTbl & strWhere & " order by nama_supplier " & strLimit
...
Private Sub cmdPagingNext_Click()
    ...
    If pagingValid(cboPerPage, txtPagingPos + 1, "supplier", strWhere) Then
        txtPagingPos = Val(txtPagingPos) + 1
    End If
    cmdReload_Click
End Sub
...
On Error GoTo jikaError                     'apabila terjadi error maka proses akan dilompati ke jikaError:
    ...
    namaTabel = "supplier"                  'set nilai dari namaTabel adalah kategori (nama tabel didatabase yang akan diproses)
    ...
    'untuk kondisi sebaiknya kita tampung dalam sebuah variabel, misal strWhere.
        Dim strWhere As String
        strWhere = "nama_supplier =" & modulGencil.AntiSQLiWithQuotes(txtNama)  'gunakan fungsi antiSqLiWithQuotes untuk keamanan
                                                                                'selengkapnya silahkan cari di google apa itu SQL injection
        'jika nama kategori sudah ada
        If modulGencil.isDuplicate(namaTabel, "nama_supplier", strWhere) Then
            'tampilkan pesan kalo nama kategori sudah terdaftar
            MsgBox "Nama Supplier sudah terdaftar", vbInformation, "Validasi"
            Me.txtNama.SetFocus                 'mengarahkan kursor ke txtnama agar bisa langsung diisi
            Exit Sub                            'keluar dari sub cmdSave
        End If
    ...
    If response = vbYes Then
        ...
        nilaiValue(2) = modulGencil.AntiSQLi(txtCp)
        nilaiValue(3) = modulGencil.AntiSQLi(txtAlamat)
...
On Error GoTo jikaError     'error handling seperti proses simpan
    ...
    Dim strWhere As String
    strWhere = "nama_supplier=" & modulGencil.AntiSQLiWithQuotes(Me.txtNama) & _
               " AND supplier_id NOT in (" & modulGencil.AntiSQLiWithQuotes(str(tmpID)) & ")"  'str(tmpID) adalah mengubah tipe data tmpID yang semula adalah variant jadi string
    ...
    If response = vbYes Then
        namaKolom(0) = "supplier_id"
        nilaiValue(0) = modulGencil.AntiSQLi(str(tmpID))
        
        namaKolom(1) = "nama_supplier"
        nilaiValue(1) = modulGencil.AntiSQLi(Me.txtNama)
        
        namaKolom(2) = "cp_supplier"
        nilaiValue(2) = modulGencil.AntiSQLi(Me.txtCp)
        
        namaKolom(3) = "alamat"
        nilaiValue(3) = modulGencil.AntiSQLi(Me.txtAlamat)
...
On Error GoTo jikaError
    ...
    Dim strWhere As String
    strWhere = "supplier_id=" & modulGencil.AntiSQLiWithQuotes(str(tmpID))
    'konfirmasi
